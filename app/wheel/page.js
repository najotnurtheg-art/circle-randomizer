...
export default function WheelPage() {
  ...
  const draw = (a, segs) => {
    ...
    const s = segs.length ? segs : [{ type: 'item', name: 'Admin /admin/items ga sovg‘a qo‘shing' }];
  ...
  const getStore = async () => {
    try {
      const url = (typeof window !== 'undefined' ? window.location.origin : '') + '/api/store/list';
      const r = await fetch(url, { cache: 'no-store' });
      if (!r.ok) return setStoreItems([]);
      const j = await r.json();
      Array.isArray(j) ? setStoreItems(j) : setStoreItems([]);
    } catch { setStoreItems([]); }
  };
  ...
  // spin animation (SYNC RAF callback; credit AFTER stop)
  const startSharedSpin = (spin) => {
    ...
    const run = (t) => {
      ...
      if (e < 1) {
        ...
        // CREDIT + popup AFTER stop (non-blocking)
        (async () => {
          try {
            ...
            if (r.ok) {
              ...
              if (seg?.type === 'item')
                ...
                setPopup({ text: `'${spin.username}' siz +${seg.amount} tangalarni yutib oldingiz🎉` });
              else setPopup({ text: `'${spin.username}' uchun yana bir aylantirish!` });
            }
          } catch {}
          getLatestWins(); getFeatured(); getStore();
        })();
      }
    };
    rafRef.current = requestAnimationFrame(run);
      ...
      <div className="wrap">
        ...
        <div className="side">
          <div className="card" style={{ marginBottom: 16 }}>
            ...
            <div style={{ fontSize: 12, opacity: 0.8, marginTop: 6 }}>Admin bu ro‘yxatni kerak bo‘lsa keyin kengaytirishi mumkin.</div>
          </div>
              ...
              <ul style={{ margin: 0, paddingLeft: 0, listStyle: 'none', lineHeight: 1.6 }}>
                {latestWins.map((w) => (
                  <li key={w.id} style={{ display: 'flex', justifyContent: 'space-between', gap: 8, padding: '6px 0', borderBottom: '1px dashed #374151' }}>
                    ...
                    <span title={new Date(w.when).toLocaleString()} style={{ opacity: 0.8, fontSize: 12 }}>{new Date(w.when).toLocaleTimeString()}</span>
                    <b style={{ maxWidth: '35%', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{w.prize}</b>
                  </li>
                ))}
              </ul>
            )}
            <div style={{ fontSize: 12, opacity: 0.8, marginTop: 6 }}>Ro‘yxat har 4 soniyada yangilanadi.</div>
          </div>
        </div>
