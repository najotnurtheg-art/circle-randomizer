...
export default function WheelPage() {
  ...
  const [state, setState] = useState({ status: 'IDLE', userId: null, username: null });
  ...
  const draw = (a, segs) => {
    ...
    const s = segs.length ? segs : [{type:'item', name:'Admin items qo‘shing'}];
  ...
  const getSegments = async (w) => {
    ...
    const j = await r.json(); if (j.segments) setSegments(j.segments);
  };
      ...
      <div className="wrap">
        <div className="side">
          <div className="card" style={{marginBottom:16}}>
            ...
            <div style={{fontSize:12, opacity:.8, marginTop:6}}>Admin ro‘yxatni keyin kengaytirishi mumkin.</div>
          </div>

          <div className="card">
            <div className="title">Oxirgi 5 yutuq</div>
            {latestWins.length === 0 ? (
              <div style={{opacity:.8}}>Hali yutuqlar yo‘q.</div>
            ) : (
            ...
            <div style={{fontSize:12, opacity:.8, marginTop:6}}>Har 4 soniyada yangilanadi.</div>
          </div>
        </div>
        ...
        <div className="side side-right">
          <div className="card" style={{marginBottom:16}}>
            ...
            {featuredUsers.length === 0 ? (
              <div style={{opacity:.8}}>Hozircha bo‘sh.</div>
            ) : (
              <ul style={{margin:0, paddingLeft:0, listStyle:'none', lineHeight:1.6}}>
                {featuredUsers.map(u=>(
            ...
            <div style={{fontSize:12, opacity:.8, marginTop:6}}>Har 3 soniyada yangilanadi.</div>
          </div>
              ...
              <ul style={{margin:0, paddingLeft:0, listStyle:'none', display:'grid', gap:8}}>
                {storeItems.map(it=>(
                  <li key={it.id} style={{display:'flex', alignItems:'center', gap:10}}>
                    ...
                    {/* buying stayed in your previous code; if needed, hook back the buy() action */}
                  </li>
                ))}
              </ul>
            ) : (
